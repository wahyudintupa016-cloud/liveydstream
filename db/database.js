...
function createTables() {
  return new Promise((resolve, reject) => {
    db.serialize(() => {
      ...
      db.run(`ALTER TABLE streams ADD COLUMN is_daily INTEGER DEFAULT 0`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding is_daily column to streams:', err.message);
        }
      });

      db.run(`ALTER TABLE streams ADD COLUMN channel_name TEXT`, (err) => {
        if (err && !err.message.includes('duplicate column name')) {
          console.error('Error adding channel_name column to streams:', err.message);
        }
      });
